let handler = async (m, { conn, args, command }) => {
    ...
    if (command.endsWith('all') || command.endsWith('semua')) {
        for (let i = 0; i < chat.length; i++) { // For loops
            await m.reply('Byee👋, SangBot leave the Group as per request of Management Team', chat[i].jid)
        ...
        await m.reply('Byee👋, SangBot leave the Group as per request of Management Team', args[0])
        await conn.groupLeave(args[0])
        await m.reply('Success!')
    } else {
        if (!m.isGroup) return global.dfail('group', m, conn)
        await m.reply('Byee👋, SangBot leave the Group as per request of Management Team', m.chat) // WKWKW all the messages are the same. not creative :v
